          ...
          if (
            ...
            parameters.forEach((p) => {
              if (isValid && p.isActive) {
                ...
                if (isValid && window.$("#node-input-" + p.id).length)
                  isValid = window
                    .$("#node-input-" + p.id)
                    .typedInput("validate");
              }
    ...
    oneditsave: function () {
      ...
      if (clone.saveTypedInputAgain) {
        clone.saveTypedInputAgain.forEach(({ index, id }) => {
          clone.parameters[index].value = window
            .$("#node-input-" + id)
            .typedInput("value");
        });
      }
...
<script>
  ...
  const createApi = async () => {
    try {
      ...
      // save old parameter objects (swagger version <0.2) - changed from object to array objects
  ...
  $: if (node.operation) {
    ...
    if (apiList?.[node.api]?.[node.operation]?.requestBody?.content) {
      contentTypes = Object.keys(
        apiList[node.api][node.operation].requestBody.content
      );
    } else {
...
<div id="swagger">
  <Input bind:node prop="name" placeholder="Swagger" />
  <Input bind:node prop="container" placeholder="Container" />
  <div class="form-row">
  ...
  <div style="display: flex; align-items: baseline;">
    <Input bind:node prop="openApiUrl" label="URL" />
    <span> <Button icon="edit" label="read" on:click={createApi} /> </span>
  </div>
    ...
    <Select bind:node prop="operation">
      ...
      {#each Object.entries(operations) as [key]}
        {#if node.operation === operations[key].operationId}
          <option value={operations[key].operationId} selected
            >{operations[key].summary}</option
          >
        {:else}
          <option value={operations[key].operationId}
            >{operations[key].summary}</option
          >
        {/if}
      {/each}
      ...
      <div class:required={param.required} style="display:flex;">
        <div style="min-width: 99px;">
          <Input
            ...
            on:change={(e) =>
              (node.parameters[index].isActive = e.detail.value)}
          />
        </div>
      </div>
        ...
        <TypedInput
          ...
          on:change={(e) => {
            ...
            if (
              ...
              e.detail.value.toString() !== "[object Object]"
            ) {
...
<style>
  :global(#swagger .required, #swagger .required label) {
    font-weight: bold !important;
  }
  :global(#swagger .label) {
    width: 104px;
  }
  :global(#swagger .parameterInput label) {
    width: 17px !important;
  }
  :global(#swagger #node-input-openApiUrl) {
    width: 100%;
  }
  :global(#swagger .sir-input) {
    margin-right: 5px !important;
  }
  /*
  :global(#swagger .sir-button ) {
    height: 10px;
  }  */
  :global(#swagger .red-ui-editableList-item-content div) {
    margin-top: 0px !important;
    margin-bottom: 0px !important;
  }
  :global(#swagger .red-ui-editableList-container) {
    min-height: 300px;
    height: 100% !important;
    overflow-y: hidden !important;
  }
  :global(#swagger .red-ui-editableList .red-ui-typedInput-container) {
    width: 90% !important;
  }
  :global(#swagger .nodeError) {
    color: red;
  }
</style>
